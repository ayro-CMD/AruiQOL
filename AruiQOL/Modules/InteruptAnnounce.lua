...
initFrame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGIN" then
        ...
        local CHAT_THROTTLE = 1.0  -- minimum 1s between chat messages
            ...
            if #pendingChat > 0 then
                ...
                for i, data in ipairs(pendingChat) do
                    if now - lastChatTime >= CHAT_THROTTLE then
                        ...
                        local ok, err = pcall(SendChatMessage, plainMsg, data.channel)
                        if not ok then
        ...
        interruptFrame:SetScript("OnEvent", function(self, ev, ...)
            ...
            local ok, err = pcall(function()
                ...
                if type(args[3]) == "string" then
                    -- Ascension-style: no hideCaster field
                    combatEvent   = args[2]
                    sourceGUID    = args[3]
                    sourceName    = args[4]
                    sourceFlags   = args[5]
                ...
                elseif args[1] then
                    -- Standard 3.3.5: arg 3 = hideCaster (boolean/nil)
                    combatEvent   = args[2]
                    sourceGUID    = args[4]
                    sourceName    = args[5]
                    sourceFlags   = args[6]
            ...
            local spellColored  = string.format("|cff%s[%s]|r", SPELL_COLOR, extraSpellName or spellName or "Spell")
