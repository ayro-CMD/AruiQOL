...
version:SetText("v1.0 |cffb188ffAYRO|r")
...
miniButton:SetNormalTexture("Interface\\Icons\\INV_Misc_Coin_01")
...
local function InitDB()
    ...
    if not AruiQOLDB.InterruptAnnounce then
        AruiQOLDB.InterruptAnnounce = {
            enabled = false, output = "Auto", verbose = true, announceSelf = true,
        }
    end

    if not AruiQOLDB.Redeemer then
        AruiQOLDB.Redeemer = {
    ...
    if not AruiQOLDB.TrackOMatique then
        AruiQOLDB.TrackOMatique = {
