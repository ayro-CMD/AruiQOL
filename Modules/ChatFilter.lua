...
local function MatchesChannelLFG(event, msg, channelName, ...)
    ...
    if db.filterWorldLFG and channelName and string.find(string.lower(channelName), "world", 1, true) then
...
local function MatchesBoostFilter(msg)
    ...
    local lowerMsg = string.lower(msg)
    local boostWords = { "wts", "wtb", "selling", "boost", "boosting", "carry", "gdkp", "pilot", "piloted", "cheap", "price", "service" }
...
local function MatchesGuildFilter(msg)
    ...
    local lowerMsg = string.lower(msg)
    local guildWords = { "guild", "community", "recruit", "recruiting", "roster", "lf members", "lf guild", "new guild", "apply", "core group", "static group", "raid team" }
...
local function ChatFilter_OnEvent(self, event, msg, player, language, channelName, ...)
    ...
    if IsSpamBurst(player) then AddLog(player, msg, "Spam Burst") return true end

    local matched, trigger = MatchesCustomKeywords(msg)
    if matched then AddLog(player, msg, trigger) return true end
