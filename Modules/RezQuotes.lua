-- ============================================================
-- Arui QOL - RezQuotes Module
-- ============================================================

local rezQuotesFrame = nil
local PET_NAME = nil
local hasSoulstone = false

local function SendQuotes(chatMessage, target)
    if not AruiQOLDB or not AruiQOLDB.RezQuotes then return end
    local db = AruiQOLDB.RezQuotes
...
initFrame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGIN" then
        ...
        rezQuotesFrame = CreateFrame("Frame")
        rezQuotesFrame:RegisterEvent("UNIT_SPELLCAST_SENT")
        rezQuotesFrame:RegisterEvent("PLAYER_DEAD")
        rezQuotesFrame:RegisterEvent("PLAYER_ALIVE")

        rezQuotesFrame:SetScript("OnEvent", function(self, ev, arg1, arg2, arg3, arg4)
            if not AruiQOLDB or not AruiQOLDB.RezQuotes or not AruiQOLDB.RezQuotes.enabled then return end
