...
local function DoRepair()
    ...
    if not CanMerchantRepair or not CanMerchantRepair() then
        ...
        if repairAttempt < MAX_REPAIR_ATTEMPTS then
            ...
            C_Timer.After(0.5, DoRepair)
        else
            print("|cffff4444[AruiQOL]|r Auto-repair: vendor cannot repair (or data not ready)")
        end
        return
    end
    ...
    if not repairCost or repairCost == 0 then
        print("|cff88ccff[AruiQOL]|r Auto-repair: nothing to repair")
        return
    end
    ...
    if db.useGuildRepair then
        if CanGuildBankRepair and CanGuildBankRepair() then
            ...
            if GetGuildBankMoney then
                guildMoney = GetGuildBankMoney() or 0
            end
            if guildMoney >= repairCost then
                RepairAllItems(1)  -- 1 = guild bank on 3.3.5
                print("|cff88ccff[AruiQOL]|r Repaired with guild bank: " .. FormatMoney(repairCost))
                return
            end
        end
    end
    ...
    -- Normal repair
    -- On 3.3.5: RepairAllItems() with no arg repairs from personal gold
    -- On some 3.3.5 builds: RepairAllItems(false) might not work
    RepairAllItems()
    print("|cff88ccff[AruiQOL]|r Equipment repaired: " .. FormatMoney(repairCost))
end
...
initFrame:SetScript("OnEvent", function(self, event)
    if event == "PLAYER_LOGIN" then
        ...
        merchantFrame:SetScript("OnEvent", function()
            ...
            -- Auto Repair with retry logic for 3.3.5
            if db.autoRepair then
                repairAttempt = 0
                DoRepair()
            end

            -- Auto Sell: small delay to ensure repair finished first
